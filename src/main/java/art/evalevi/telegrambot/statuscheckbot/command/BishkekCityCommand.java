...
public class BishkekCityCommand implements Command {

    private static final String SELECTED_CITY_INFO = "<code>Документы поданы в г.%s\n\nДалее отправьте номер заявления...</code>";

    private final SendMessageService sendMessageService;

    public BishkekCityCommand(SendMessageService sendMessageService) {
        this.sendMessageService = sendMessageService;
    }

    @Override
    public void execute(Update update) {
        sendMessageService.sendMessage(getChatId(update), String.format(SELECTED_CITY_INFO, City.BISHKEK.name));
    }
}
